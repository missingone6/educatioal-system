...
(function () {
    ajax("post", "http://www.xinill.cn:80/user/inform/getAll", ``, function (xml) {
        ...
        if (res.status === 0) {
            ...
            insertInf(res.data.basicInf, document.getElementsByClassName('basicInf')[0], basic);
            insertInf(res.data.studentStatusInf, document.getElementsByClassName('statusInf')[0], studentStatus);
            insertInf(res.data.otherInf, document.getElementsByClassName('otherInf')[0], other);
        }
        else {
            alert('查询数据失败，请联系管理员\n' + res.msg);
